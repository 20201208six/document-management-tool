  ...
  <div class="new-script-predict">
    <div class="nsp-layout">
      ...
      <div class="nsp-main">
        ...
        <div class="nsp-predict-row">
          ...
          <div class="nsp-card nsp-input">
            ...
            <p class="nsp-card-desc">
              基于历史样本库中的文稿与点赞数据，通过7维评分体系（锚定Likert
              1-5量表）和相似度匹配，预估新文稿的点赞量区间。<br /><span
                style="color: #1a4cff"
                >盲预测模式：AI
                预测时看不到参考样本的实际点赞量，确保预测客观。</span
              >
            </p>

            <div class="nsp-input-row">
              <el-select
                v-model="predPlatform"
                size="default"
                class="nsp-select"
              >
                <el-option
                  v-for="(c, k) in PLATFORM_CONFIG"
                  :key="k"
              ...
              <span class="nsp-sample-hint" v-if="store.scriptRecords.length">
                样本库 {{ store.scriptRecords.length }} 条 · 同平台
                {{ samePlatformCount }} 条
              </span>
            ...
            <div class="nsp-actions">
              <button
                class="nsp-btn nsp-btn-primary"
                @click="handlePredict"
                :disabled="!predContent.trim() || store.isPredicting"
              >
                {{ store.isPredicting ? "⏳ 分析中…" : "🔮 开始预测" }}
              </button>
              <button
                class="nsp-btn nsp-btn-ghost"
                @click="handleClear"
                :disabled="!predContent.trim()"
              >
                清空
              </button>
            </div>
        ...
        <template v-if="store.lastPrediction && !store.isPredicting">
          ...
          <div v-if="viewingHistoryId" class="nsp-viewing-bar">
            ...
            <div class="nsp-vb-actions">
              ...
              <button class="nsp-vb-back" @click="clearViewingHistory">
                ✕ 返回当前
              </button>
            </div>
          </div>
          ...
          <PredictionDashboard
            ...
            :suggestions="
              store.lastPrediction.suggestions.map((s) => cleanBullet(s))
            "
            :counterfactuals="
              (store.lastPrediction.counterfactuals || []).map((c) =>
                cleanBullet(c),
              )
            "
            :account-advice-items="accountAdviceItems"
            @open-account-settings="$emit('openAccountSettings')"
          />
        </template>
        ...
        <!-- 空状态 -->
        <div
          v-if="!store.lastPrediction && !store.isPredicting"
          class="nsp-empty"
        >
          <div class="nspe-icon">📝</div>
          <span class="nspe-title">尚未预测</span>
          <span class="nspe-desc"
            >在上方输入口播文稿，AI 将基于
            {{ store.scriptRecords.length || 0 }} 条历史样本进行盲预测</span
          >
        </div>
        ...
        <div class="nsp-bump-panel" v-if="store.bumpSuggestion">
          <div class="nsbp-header">
            ...
            <span class="nsbp-time">{{
              store.bumpSuggestion.generatedAt
            }}</span>
          </div>
          <div
            class="nsbp-reasoning"
            v-for="(line, i) in store.bumpSuggestion.reasoning"
            :key="'br' + i"
          >
            {{ line }}
          </div>
          <div class="nsbp-weights">
            ...
            >
              ...
              <span class="nsbpw-new"
                >{{ store.bumpSuggestion.weights[d.key] || 0 }}%</span
              >
            </div>
          </div>
          <!-- 权重审核结果 -->
          <div class="nsbp-validation" v-if="store.weightValidationResult">
            <div class="nsbpv-header">
              <span class="nsbpv-icon">{{
                store.weightValidationResult.passed ? "✅" : "⚠️"
              }}</span>
              <span class="nsbpv-title">{{
                store.weightValidationResult.passed ? "审核通过" : "审核拦截"
              }}</span>
            </div>
            <p class="nsbpv-recommendation">
              {{ store.weightValidationResult.recommendation }}
            </p>
            <div class="nsbpv-summary">
              <span
                >验证池 {{ store.weightValidationResult.totalPairs + 1 }} 条样本
                → {{ store.weightValidationResult.totalPairs }} 对</span
              >
              <span
                >旧权重正确率
                <b
                  :class="
                    store.weightValidationResult.oldRate >= 0.8 ? 'ok' : 'ng'
                  "
                  >{{
                    Math.round(store.weightValidationResult.oldRate * 100)
                  }}%</b
                ></span
              >
              <span
                ...
                <b
                  :class="
                    store.weightValidationResult.newRate >= 0.8 ? 'ok' : 'ng'
                  "
                  >{{
                    Math.round(store.weightValidationResult.newRate * 100)
                  }}%</b
                ></span
              >
            </div>
            <details class="nsbpv-details">
              ...
              <div
                class="nsbpv-pair"
                v-for="(p, i) in store.weightValidationResult.pairs"
                :key="i"
              >
                <span class="nsbpv-pair-idx">#{{ i + 1 }}</span>
                <span class="nsbpv-pair-content">
                  <span :class="p.oldOk ? '' : 'error'"
                    >旧 {{ p.aOld }} ≥ {{ p.bOld }}</span
                  >
                  <span class="nsbpv-arrow">→</span>
                  <span :class="p.newOk ? '' : 'error'"
                    >新 {{ p.aNew }} ≥ {{ p.bNew }}</span
                  >
                </span>
                <span
                  class="nsbpv-pair-likes"
                  :class="p.oldOk && p.newOk ? '' : 'warn'"
                >
                  {{
                    p.oldOk ? (p.newOk ? "✓✓" : "✓✗") : p.newOk ? "✗✓" : "✗✗"
                  }}
                </span>
                <span class="nsbpv-pair-likes-num"
                  >{{ p.aLikes }} vs {{ p.bLikes }}赞</span
                >
              </div>
            </details>
            <!-- 跨模型审计结果 -->
            <div class="nsbpv-cross" v-if="store.crossAuditWeightResult">
              <div class="nsbpv-cross-header">
                <span>{{
                  store.crossAuditWeightResult.passed ? "🤖" : "🤖⚠️"
                }}</span>
                <span class="nsbpv-cross-label">跨模型审计</span>
                <span
                  :class="store.crossAuditWeightResult.passed ? 'ok' : 'ng'"
                  >{{
                    store.crossAuditWeightResult.passed
                      ? "双审通过"
                      : "外部驳回"
                  }}</span
                >
              </div>
              <p class="nsbpv-cross-verdict">
                {{ store.crossAuditWeightResult.verdict }}
              </p>
              <p
                class="nsbpv-cross-advice"
                v-if="store.crossAuditWeightResult.advice"
              >
                {{ store.crossAuditWeightResult.advice }}
              </p>
            </div>
          </div>
          <div class="nsbp-actions">
            <button class="nsbpa-accept" @click="store.applyBumpWeights()">
              接受并升级公式
            </button>
            <button
              class="nsbpa-dismiss"
              @click="store.dismissBumpSuggestion()"
            >
              忽略
            </button>
          </div>
      ...
      <div class="nsp-sidebar">
          ...
          <div v-show="showSideGen" class="nsp-sg-panel">
            ...
            <div v-if="generatedContent && !genLoading" class="nsp-sg-result">
              <div class="nsp-sg-result-head">
                ...
                <span class="nspsgrh-length"
                  >{{ generatedContent.length }} 字</span
                >
              </div>
        ...
        <!-- 偏差趋势警告 -->
        <div
          class="nsp-deviation-warn-side"
          v-if="store.deviationTrend"
          :class="warnClass"
        >
          <div
            class="nsdw-title-row"
            @click="showDeviationPanel = !showDeviationPanel"
          >
            <span class="nspsbt-toggle" :class="{ open: showDeviationPanel }"
              >▶</span
            >
            ...
            <button
              v-if="
                !showDeviationPanel &&
                store.deviationTrend.needsBump &&
                !store.isGeneratingBump
              "
          ...
          <template v-if="showDeviationPanel">
            <div class="nsdw-stats-row">
              <div class="nsdw-stat-item">
                <span
                  class="nsdw-stat-val"
                  :class="{
                    low: (store.deviationTrend.headlineHitRate ?? 0) < 50,
                  }"
                  >{{ store.deviationTrend.headlineHitRate ?? "--" }}%</span
                >
              ...
              <div class="nsdw-stat-item">
                <span
                  class="nsdw-stat-val"
                  :class="{ high: (store.deviationTrend.missRate ?? 0) > 0 }"
                  >{{ store.deviationTrend.missRate ?? "--" }}%</span
                >
                <span class="nsdw-stat-label">脱靶率</span>
              </div>
              <div class="nsdw-stat-item">
                <span class="nsdw-stat-val"
                  >{{ store.deviationTrend.avgDev > 0 ? "+" : ""
                  }}{{ store.deviationTrend.avgDev }}%</span
                >
            ...
            <span v-if="store.isGeneratingBump" class="nsdw-bumping"
              >⏳ 分析中…</span
            >
          </template>
        </div>
        ...
        <div class="nspsb-card" v-if="store.evolutionMetrics">
          <div class="nspsb-title-row">
            <span
              class="nspsbt-toggle"
              :class="{ open: showEvolutionPanel }"
              @click="showEvolutionPanel = !showEvolutionPanel"
              >▶</span
            >
            ...
            >
              ...
              <span class="nspsba-text">{{
                store.autoCalibrateEnabled ? "开" : "关"
              }}</span>
            </label>
          </div>
          <template v-if="showEvolutionPanel">
            <div class="nspsb-stats">
              <div class="nspsbs-item">
                <span class="nspsbs-val">{{
                  store.evolutionMetrics.totalScripts
                }}</span>
                <span class="nspsbs-label">样本总数</span>
              </div>
              <div class="nspsbs-item">
                <span class="nspsbs-val">{{
                  store.evolutionMetrics.totalPredictions
                }}</span>
                <span class="nspsbs-label">预测次数</span>
              </div>
              <div class="nspsbs-item">
                <span class="nspsbs-val">{{
                  store.evolutionMetrics.totalRetros
                }}</span>
                <span class="nspsbs-label">已复盘</span>
              </div>
              <div class="nspsbs-item">
                <span class="nspsbs-val">{{
                  store.evolutionMetrics.weightChanges
                }}</span>
            ...
            <div
              class="nspsb-tier"
              v-if="store.evolutionMetrics.sampleMilestone"
            >
              🏆 {{ store.evolutionMetrics.sampleMilestone }}
            </div>
            ...
            <div
              class="nspsb-trend"
              v-if="
                store.evolutionMetrics.trendDescription !== '暂无足够复盘数据'
              "
            >
              ...
              <div class="nspsbt-row">
                <div class="nspsbtr-item">
                  ...
                  <span class="nspsbtri-val"
                    >{{ store.evolutionMetrics.earlyHitRate ?? "--" }}%</span
                  >
                </div>
                ...
                <div class="nspsbtr-item">
                  ...
                  <span
                    class="nspsbtri-val"
                    :class="
                      (store.evolutionMetrics.recentHitRate ?? 0) >
                      (store.evolutionMetrics.earlyHitRate ?? 0)
                        ? 'up'
                        : 'down'
                    "
                    >{{ store.evolutionMetrics.recentHitRate ?? "--" }}%</span
                  >
                </div>
                <span
                  class="nspsbt-diff"
              ...
              <div class="nspsbt-desc">
                {{ store.evolutionMetrics.trendDescription }}
              </div>
            </div>
            ...
            >
              <span class="nspsba-dot"></span>
              <span class="nspsba-text">{{
                store.autoCalibrateEnabled ? "自动校准：开" : "自动校准：关"
              }}</span>
            </label>
            ...
            <div class="nspsb-wlog" v-if="store.weightChangeLog.length > 0">
              ...
              <div
                class="nspsbwl-item"
                v-for="w in store.weightChangeLog.slice(0, 5)"
                :key="w.id"
              >
                ...
                <div
                  v-if="expandedWLogIds.includes(w.id)"
                  class="nspsbwli-diffs"
                >
                  <div
                    v-if="getWeightDiffs(w).length === 0"
                    class="nspsbwlid-empty"
                  >
                    各维度无变化
                  </div>
                  <div
                    v-for="d in getWeightDiffs(w)"
                    :key="d.key"
                    class="nspsbwlid-row"
                  >
        ...
        <!-- 预测历史 -->
        <div
          class="nsp-history-section"
          v-if="store.predictionHistory.length > 0"
        >
          <div class="nsp-hist-header">
            📜 预测历史 ({{ store.predictionHistory.length }} 条)
            <span v-if="unreviewedCount > 0" class="nsp-hist-unreviewed"
              >{{ unreviewedCount }} 条未复盘</span
            >
          </div>
          <div class="nsp-history-list">
            <div
              ...
              :class="{
                active: viewingHistoryId === entry.id,
                reviewed: entry.actualLikes != null,
              }"
              @click="handleViewHistory(entry)"
            >
              <div class="nsp-hist-main">
                <span class="nsp-hist-platform">{{
                  PLATFORM_CONFIG[entry.platform]?.icon
                }}</span>
                <span class="nsp-hist-text"
                  >{{ entry.content.slice(0, 50)
                  }}{{ entry.content.length > 50 ? "..." : "" }}</span
                >
              </div>
              <div class="nsp-hist-meta">
                <span class="nsp-hist-pred"
                  >{{ fmt(entry.result.minLikes) }}~{{
                    fmt(entry.result.maxLikes)
                  }}</span
                >
                <span
                  v-if="entry.actualLikes != null"
                  class="nsp-hist-actual"
                  :class="
                    entry.deviation && entry.deviation > 0 ? 'over' : 'under'
                  "
                >
                ...
                <span class="nsp-hist-time">{{
                  entry.predictedAt.slice(5, 16)
                }}</span>
              </div>
              <button
                class="nsp-hist-del"
                @click.stop="handleDeleteHistory(entry)"
                title="删除此条预测"
              >
                <svg
                  viewBox="0 0 24 24"
    ...
    <el-dialog
      v-model="showRetro"
      :title="isReRetro ? '🔄 更新数据（不重复入库）' : '📊 复盘录入'"
      width="460px"
    ...
    >
      <div class="retro-body" v-if="retroEntry">
        <div class="retro-pred">
          ...
          <strong
            >{{ fmt(retroEntry.result.minLikes) }} ~
            {{ fmt(retroEntry.result.maxLikes) }}</strong
          >
          <span class="retro-time">（{{ retroEntry.predictedAt }}）</span>
        </div>
        <div class="retro-content-preview">
          {{ retroEntry.content.slice(0, 100)
          }}{{ retroEntry.content.length > 100 ? "..." : "" }}
        </div>
        <div v-if="isReRetro" class="retro-rero-hint">
          ⚠️ 仅更新预测历史中的数据，不会重复存入样本库
        </div>

        <!-- 主平台 -->
        <div class="retro-form">
          <label class="retro-label"
            >{{ PLATFORM_CONFIG[retroEntry.platform]?.icon }}
            {{ retroEntry.platform }} · 实际点赞量：</label
          >
          <el-input-number
            v-model="retroLikes"
            :min="0"
        ...
        <div
          ...
          v-if="!isReRetro && otherPlatformsForRetro.length > 0"
        >
          <div
            class="retro-multi-toggle"
            @click="retroMultiPlatform = !retroMultiPlatform"
          >
            <span class="retro-multi-label">多平台发布</span>
            <span class="retro-multi-hint">{{
              retroMultiPlatform
                ? "已开启 · 可为各平台分别填写数据"
                : "开启后可为各平台分别填写点赞/播放量"
            }}</span>
            <span
              class="retro-multi-switch"
              :class="{ on: retroMultiPlatform }"
            ></span>
          </div>
        </div>
          ...
          >
            ...
            <template v-if="retroPlatformData[p.key].enabled">
              <div style="display: flex; gap: 8px; margin-top: 4px">
                ...
                />
                <el-input-number
                  v-model="retroPlatformData[p.key].views"
                  :min="0"
                  :max="999999999"
                  :step="100"
                  size="small"
...
const accountAdviceItems = computed(() => {
  ...
  return topN.map((item, idx) => {
    ...
    const correlation =
      typeof item.correlation === "number" ? item.correlation : 0;
...
async function handleGenerate() {
  ...
  try {
    generatedContent.value = await store.generateContent(
      genTopic.value.trim(),
      genPlatform.value,
      genTargetLikes.value || undefined,
    );
  } catch (e: any) {
...
const retroEntryForCurrent = computed(() => {
  ...
  return (
    store.predictionHistory.find((e) => e.id === viewingHistoryId.value) ?? null
  );
});
...
// 多平台数据
const allPlatformKeys = Object.keys(PLATFORM_CONFIG).filter(
  (k) => k !== "全部",
) as Platform[];
const retroPlatformData = reactive<
  Record<string, { enabled: boolean; likes: number; views: number }>
>(
  Object.fromEntries(
    allPlatformKeys.map((k) => [k, { enabled: false, likes: 0, views: 0 }]),
  ),
);
const otherPlatformsForRetro = computed(() =>
  retroEntry.value
    ? allPlatformKeys
        .filter((k) => k !== retroEntry.value!.platform)
        .map((k) => ({ key: k }))
    : [],
);
...
function handleRetro() {
  ...
  let additionalPlatforms:
    | Array<{ platform: Platform; likes: number; views?: number }>
    | undefined;
  if (!isReRetro.value && retroMultiPlatform.value) {
    const extras = allPlatformKeys
      .filter(
        (k) =>
          k !== retroEntry.value!.platform &&
          retroPlatformData[k].enabled &&
          retroPlatformData[k].likes > 0,
      )
      .map(
        (k) =>
          ({
            ...
            views:
              retroPlatformData[k].views > 0
                ? retroPlatformData[k].views
                : undefined,
  ...
  retroLikes.value = null;
  retroViews.value = null;
  retroNote.value = "";
  retroMultiPlatform.value = false;
...
.nsp-improve-btn:hover:not(:disabled) {
  background: #d1fae5;
  border-color: #059669;
}
.nsp-improve-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.nsp-btn-primary:hover:not(:disabled) {
  background: #0f3fd9;
}
.nsp-btn-ghost {
  background: #f5f7fb;
...
.nspe-auto-toggle:hover {
  border-color: #9ca3af;
}
.nspe-auto-toggle.on:hover {
  border-color: #10b981;
}
.npeat-dot {
  width: 7px;
...
.nspe-auto-toggle.on .nspeat-dot {
  background: #10b981;
}
...
.nsp-history-item:hover {
  background: #f0f4ff;
}
.nsp-history-item.active {
  background: #eef3ff;
  border-color: #3b82f6;
}
.nsp-history-item.reviewed {
  background: #f9fafb;
}
.nsp-hist-del {
  position: absolute;
...
.nsp-history-item:hover .nsp-hist-del {
  opacity: 1;
}
.nsp-hist-del:hover {
  color: #ef4444;
...
.nsp-hist-actual.over {
  color: #0f7b3a;
}
.nsp-hist-actual.under {
  color: #f56c6c;
}
.nsp-hist-pending {
  font-size: 11px;
...
.nsp-hist-retro-btn:hover {
  background: #d97706;
  border-color: #d97706;
}
.nsp-hist-retro-btn.rero {
  border-color: #bfdbfe;
  color: #2563eb;
  background: #eff6ff;
}
.nsp-hist-retro-btn.rero:hover {
  background: #dbeafe;
  border-color: #93c5fd;
}
.nsp-hist-time {
...
.nspro-value.risk-low {
  color: #0f7b3a;
}
.nspro-value.risk-mid {
  color: #e6a23c;
}
.nspro-value.risk-high {
  color: #f56c6c;
}
.nspro-value.confidence {
  font-size: 18px;
  color: #1a4cff;
}
.nspro-score .nspro-value {
  color: #1a4cff;
}
.nspro-likes .nspro-value {
  color: #e6a23c;
}
.nspro-tag {
...
.nsp-prob-fill.headline {
  background: linear-gradient(90deg, #1a4cff, #4f7dff);
}
.nsp-prob-pct {
  width: 50px;
  font-size: 13px;
...
.nsp-suggestions li::marker {
  color: #1a4cff;
}
.nsp-counter li::marker {
  color: #e6a23c;
}
...
.nsp-reason--action:hover {
  border-color: #22c55e;
  box-shadow: 0 1px 6px rgba(34, 197, 94, 0.08);
}
.nsp-reason-num--action {
...
.nsprccs-item.improved .nsprccs-val {
  color: #059669;
}
.nsprccs-arrow {
  font-size: 16px;
...
.retro-multi-switch.on {
  background: #4f46e5;
}
.retro-multi-switch.on::after {
...
.nsp-deviation-warn-side.warn-severe:hover {
  border-color: #ef4444;
}
.nsp-deviation-warn-side.warn-moderate:hover {
  border-color: #f59e0b;
}
.nsp-deviation-warn-side.warn-mild:hover {
  border-color: #3b82f6;
}
.nsdw-title-row {
  display: flex;
...
.nsbpv-pair-content span {
  display: inline-block;
  padding: 1px 4px;
  border-radius: 3px;
}
.nsbpv-pair-content span.error {
  color: #dc2626;
  background: #fef2f2;
}
.nsbpv-arrow {
...
.nsbpv-pair-likes.warn {
  color: #d97706;
}
.nsbpv-pair-likes-num {
  color: #9ca3af;
  font-size: 10px;
}
.nsbpv-cross {
...
.nsbpv-cross-header .ok {
  font-size: 11px;
  color: #16a34a;
  font-weight: 600;
}
.nsbpv-cross-header .ng {
  font-size: 11px;
  color: #dc2626;
  font-weight: 600;
}
...
.nspet-bar-fill.early {
  background: #93c5fd;
}
.nspet-bar-fill.recent {
  background: #3b82f6;
}
.nspet-bar-val {
  font-size: 11px;
...
.nspewl-trigger.reset {
  background: #f3f4f6;
  color: #6b7280;
}
.nspewl-trigger.manual {
  background: #e0f2fe;
  color: #0c4a6e;
}
.nspewl-note {
...
.nsp-sg-btn-spin {
  ...
  border-radius: 50%;
  animation: nsp-sg-spin 0.6s linear infinite;
}
@keyframes nsp-sg-spin {
  to {
...
.nsp-sg-loading-dots span:nth-child(2) {
  animation-delay: 0.15s;
}
.nsp-sg-loading-dots span:nth-child(3) {
...
.nspsbwli-trigger.bump {
  background: #ede9fe;
  color: #7c3aed;
}
.nspsbwli-trigger.reset {
  background: #e5e7eb;
  color: #6b7280;
}
.nspsbwli-trigger.manual {
  background: #dbeafe;
  color: #2563eb;
}
.nspsbwli-note {
